...
**Goal:** 让 AstrBot 插件发给 `opencode acp` 的 initialize / session / prompt 主链路重新符合 ACP v1 协议，并恢复真实 OpenCode runtime update 的正确消费。
**Requirements**: PHASE-02.3-01, PHASE-02.3-02, PHASE-02.3-03, PHASE-02.3-04, PHASE-02.3-05
**Depends on:** Phase 2
**Success Criteria** (what must be TRUE):

1. 插件发给 `opencode acp` 的 `initialize` 不再因缺少 `protocolVersion`、`clientCapabilities`、`clientInfo.version` 而报 `Invalid params`。
2. 新建/恢复会话与首次 prompt 的 payload 符合 ACP v1，`session/new` / `session/load` 显式带 `mcpServers: []`，`session/prompt` 使用 `prompt` 字段。
...
- [ ] 03-01-PLAN.md — 修好 `/oc` 的流式聊天回传与正文 chunk 合并展示
- [ ] 03-02-PLAN.md — 收敛权限确认与生命周期命令的摘要优先回显
- [ ] 03-03-PLAN.md — 为常见命令误用补齐短帮助与可复制示例
