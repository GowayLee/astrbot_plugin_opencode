...
- 默认使用中文交流，除非用户明确切换语言。
- 简单任务保持简短；复杂任务给结构化背景、约束和结论。
- 先给可执行结果，必要时再补关键原理和取舍。
- 选库或选方案时偏成熟稳定，必要时会先查文档再定。
- 调试时希望先结合上下文诊断，而不是只丢一个修复结果。
- 更在意指令遵守、范围控制和代码结构，避免多做、乱塞、把 TODO 往后推。
- 主要关注 backend、CLI、基础设施类问题；UI 只要求清晰可用，不追求过度设计。
- 遇到新内容时，先解释相关部分，再结合代码和文档理解。
